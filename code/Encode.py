...
def load_credentials() -> tuple:
    """首次从磁盘读取 credentials.json，自动补 jpushId，不存在则初始化"""
    default_jpush = ''.join(random.choices('0123456789abcdef', k=16))

    if not os.path.exists(CREDENTIALS_PATH):
        # 自动初始化空文件
        with open(CREDENTIALS_PATH, "w", encoding="utf-8") as f:
            json.dump({"uid": "", "token": "", "jpushId": default_jpush}, f, ensure_ascii=False, indent=2)
    ...
    with open(CREDENTIALS_PATH, "r+", encoding="utf-8") as f:
        ...
        jpushId = data.get("jpushId", default_jpush)

        # 如果 jpushId 不存在就写入
        if "jpushId" not in data:
            data["jpushId"] = jpushId
            f.seek(0)
            json.dump(data, f, ensure_ascii=False, indent=2)
            f.truncate()

    return uid, token, jpushId
...
def update_cached_credentials(uid: str, token: str):
    ...
    _, _, jpushId = load_credentials()
    load_credentials.__wrapped__ = lambda: (uid, token, jpushId)
    load_credentials.cache_clear()
    functools.lru_cache(maxsize=1)(load_credentials)


def get_cached_credentials() -> tuple:
    """获取当前内存缓存的 uid/token/jpushId"""
    return load_credentials()

def PhoneLoginD():
    account = str(input("输入手机号"))
    pwd = str(input("输入密码"))
    uid, token, JPUSH_ID = load_credentials()
    login_data = {
        "jPushId": JPUSH_ID,
        "account": account,
        "pwd": des_encrypt(pwd),
